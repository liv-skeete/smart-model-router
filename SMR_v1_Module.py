...
class Filter:
    ...
    class Valves(BaseModel):
        ...
        prompt_cache_timeout_seconds: int = Field(
            default=300,
            ge=0,
            description="Time in seconds to reuse previous model selection. 0 to disable.",
        )
    ...
    def _get_cache_status(self, messages: List[Dict], timeout: int) -> Dict[str, str]:
        """
        Check message history for recent router metadata and return cache status.
        Returns: {"status": "active"|"expired"|"none"|"disabled", "model": "model_name"|"none"}
        """
        if timeout <= 0:
            return {"status": "disabled", "model": "none"}
        
        # Find last message with router metadata
        for msg in reversed(messages):
            metadata = msg.get("metadata", {})
            router_data = metadata.get("router", {})
            if router_data and "timestamp" in router_data:
                elapsed = time.time() - router_data["timestamp"]
                model = router_data.get("model", "none")
                status = "active" if elapsed <= timeout else "expired"
                return {"status": status, "model": model}
        
        return {"status": "none", "model": "none"}
    ...
    async def _get_model_recommendation(
        ...
        # Get cache status for prompt enhancement
        cache_info = self._get_cache_status(messages, self.valves.prompt_cache_timeout_seconds)
        cache_status = cache_info["status"]
        last_model = cache_info["model"]
