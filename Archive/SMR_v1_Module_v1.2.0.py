...
class Filter:
    ...
    async def _get_model_recommendation(
        self, body: dict, models: List[str]
        ...
        """
        if not self.valves.router_system_prompt or not self.valves.router_system_prompt.strip():
    ...
    async def inlet(
        ...
        if isinstance(router_overrides, dict) and isinstance(
            ...
            forced_model = router_overrides["force_model"]
            forced_model = routed_map.get(forced_model)
            if forced_model:
                body.setdefault("metadata", {})
                body["metadata"]["router"] = {
                    "model": forced_model,
                ...
                self._log_message(f"Forced model '{forced_model}' -> '{forced_model}'")
                    ...
                    await __event_emitter__(
                        {
                            "type": "status",
                            "data": {
                                "description": f"Mode: {forced_model}",
                                "done": True,
                            },
                        }
                    )
                ...
                logger.warning(
                    "Forced model '%s' not found in routed_models; falling back.",
                    forced_model,
                )
        # ------------------------------------------------
        ...
        result = await self._get_model_recommendation(body, valid_models)
        if result is None:
